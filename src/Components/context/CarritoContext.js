...
export const CarritoProvider = (props) => {
    ...
    const getItemQuantity = () => {
       ...
       //total subtotal
       const subtotal = () => {
        return carrito.reduce((acum, prod) => acum += ((prod.cant * prod.precio)/ 1.18), 0).toFixed(2)
    } 

    //total igv
    const igv = () => {
        return carrito.reduce((acum, prod) => acum += ((prod.cant*prod.precio)/1.18 *0.18), 0).toFixed(2)
    }
...
//Precio total de la compra
    ...
    return (
        <CarritoContext.Provider value={{ carrito, addItem, removeItem, emptyCart, getItemQuantity, totalPrice, igv, subtotal, }} >
            {props.children}
